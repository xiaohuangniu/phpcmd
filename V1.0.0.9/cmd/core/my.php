...
class my implements CmdInterface{
	...
	public function L(){
		...
		if ($tables) {
			...
			return ['code'=>'01', 'data'=>'数据库结构获取失败'];
		}

		$html = '';
		foreach ($tables as $table){
            $html .= '<div style="width:5%;float:left">&nbsp;'.$table[0].'</div>';  
        }
		$data[] = $html;
	...
	public function LL(){
		...
		if (empty($this->command_data[2])) { return ['code'=>'01', 'data'=>'表名不允许为空']; }
		...
		$status = false;
		foreach ($tables as $table){
            if($table[0] == $this->command_data[2]){
				$status = true;
				$table  = $this->command_data[2];
				break;
			}
        }
		
		if (!$status) { return ['code'=>'01', 'data'=>'数据表不存在']; }

		# 信息包括
		# 1、 表名
		# 2、 字段名 字段类型 长度 描述 是否主键 索引
		# 3、表总记录数
		# 4、表冗余量
		...
		$data[] = "表名：{$table}";
		$html   = '';
		$html  .= '<div style="width:10%;float:left">&nbsp;字段</div>';  
		$html  .= '<div style="width:10%;float:left">&nbsp;类型</div>';  
		$html  .= '<div style="width:10%;float:left">&nbsp;默认</div>';  
		$html  .= '<div style="width:10%;float:left">&nbsp;主键</div>';  
		$html  .= '<div style="width:20%;float:left">&nbsp;注释</div>';  
		$data[] = $html;

		$res = $pdo->query("SHOW FULL COLUMNS FROM {$table}");
		...
		foreach ($info as $val){
            ...
            $html   = '';
			$html  .= '<div style="width:10%;float:left">&nbsp;'.$val['Field'].'</div>';  
			$html  .= '<div style="width:10%;float:left">&nbsp;'.$val['Type'].'</div>';  
			$html  .= '<div style="width:10%;float:left">&nbsp;'.$val['Default'].'</div>';  
			$html  .= '<div style="width:10%;float:left">&nbsp;'.$key.'</div>';  
			$html  .= '<div style="width:20%;float:left">&nbsp;'.$val['Comment'].'</div>';  
			$data[] = $html;
        }
		
		$res = $pdo->query("SELECT * FROM {$table}");
		$data[] = '总记录数：'.$res->rowCount();
