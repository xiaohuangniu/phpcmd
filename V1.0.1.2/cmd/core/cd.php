...
/*
 ...
 + Last-time    : 2017-9-15 15:50 + 小黄牛
 + Desc         : 文件夹进入操作
 +              : cd 路径         进入目录或打开文件，输入cd /直接进入配置CD_PATH的根目录
 +              : cd 文件路径 -l  遍历该文件的缓存历史记录，根据遍历出来的记录可点击还原与删除
 +              : cd -h          查看cd命令当前所在的目录
...
class cd implements CmdInterface{
	...
	public function path_file(){
		...
		if (is_file($url)) {
			...
			# 获取备份列表
			if (!empty($this->command_data[2]) && $this->command_data[2] == '-l') {
				$key_path   = 'config/user_cache/' . $_SESSION['cmd_user'] . '/' . md5($_SESSION['cmd_post_file']) . '/';
				$cache_path = $this->config_data['CD_PATH'] . 'cmd/'. $key_path;

				if (!file_exists($cache_path)) {
					return ['code' => '01','data' => '无该文件缓存记录']; 
				}
				
				if ($dh = opendir($cache_path)){
					while (($file = readdir($dh)) !== false){
						if ($file != '.' && $file != '..') {
							$data[] = $file . '<a class="getCache" style="margin-left: 50px;" url="'.$_SESSION['cmd_post_file'].'" title="'.$key_path.$file.'" type="1">还原</a><a class="getCache" style="margin-left: 30px;" url="" title="'.$key_path.$file.'" type="2">删除</a>';
						}
					}
					closedir($dh);
				}
				if (!empty($data)) { return ['code' => '00','data' => $data]; }
				return ['code' => '01','data' => '无该文件缓存记录']; 
			}else{
